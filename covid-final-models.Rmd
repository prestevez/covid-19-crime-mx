---
title: "Crime and COVID-19: Effects of changes in routine activities in Mexico City. Replication materials, v0.02"
...
This document contains the replication materials for the study "Crime and COVID-19: Effects of changes in routine activities in Mexico City". The study is available as a pre-print here: [https://doi.org/10.31235/osf.io/3jfwu](https://doi.org/10.31235/osf.io/3jfwu).

This is version 0.02 of the replication materials.
...
### year annotations
tibble(
  Variable = factor("All crimes"),
  value = c(150, 870),
  labels = c("2020", "2017-2019"),
  t = c(95, 137)
) -> year_annotations
...
daily_crime_mobility %>%
  ...
  geom_text(data = covid_annotations,
             ...
             size = 2.5,
             nudge_y = -20) +
  ### Add annotations to identify years
  geom_text(data = year_annotations,
            ...
            hjust = "right",
            inherit.aes = FALSE) +
  geom_curve(data = y20_curve,
             aes(x = x, y = y, xend = xend, yend = yend),
             inherit.aes = FALSE,
             arrow = arrow(length = unit(0.05, "npc")),
             size = 0.25,
             curvature = 0.35) +
  geom_curve(data = y17_19_curve,
             aes(x = x, y = y, xend = xend, yend = yend),
             inherit.aes = FALSE,
             arrow = arrow(length = unit(0.05, "npc")),
             size = 0.25,
  ...
  scale_color_manual(NULL,
                     values = c("gray", "gray", "gray", "black")) +
  scale_x_continuous(NULL,
                     breaks = mth_t$t_ind,
                     labels = mth_t$mth) +
  ylab(NULL) +
  expand_limits(y = 0) -> temporal_eda_comparison

ggsave(filename = "pdf-output/plots/temporal_eda_comparison.pdf",
...
Note that the selection of a deterministic or stochastic trend is based on comparing the prediction errors of the training set, not the testing set, as this represent how well the fitted model captures the pre-covid trend.
...
tibble(
  Variable = factor("All crimes"),
  value = c(350, 890),
  labels = c("Observed", "Forecast (95% CI)"),
  date = as.Date(c("2020-01-20", "2020-04-30"))
) -> linetype_annotations
...
daily_crime_mobility_2020 %>%
  ...
  geom_text(data = covid_annotations_date,
             ...
             hjust = "right",
            inherit.aes = FALSE) +
  ### Add annotations to identify linetypes
  geom_text(data = linetype_annotations,
            aes(x = date,
                y = value,
                label = labels,
                fontface = 3),
            ...
            hjust = "right",
            inherit.aes = FALSE) +
  geom_curve(data = observed_curve,
             aes(x = x, y = y, xend = xend, yend = yend),
             inherit.aes = FALSE,
             arrow = arrow(length = unit(0.05, "npc")),
             size = 0.25,
             curvature = 0.35) +
  geom_curve(data = forecast_curve,
             aes(x = x, y = y, xend = xend, yend = yend),
             inherit.aes = FALSE,
             arrow = arrow(length = unit(0.05, "npc")),
             size = 0.25,
  ...
  xlab(NULL) +
  # expand y axis to 0
  expand_limits(y = 0) -> arima_forecasts_plot

ggsave(filename = "pdf-output/plots/arima_forecasts_plot.pdf",
...
ggsave(filename = "pdf-output/plots/arima_forecasts_errors_plot.pdf",
...
ggsave(filename = "pdf-output/plots/arima_change_plot1.pdf",
...
ggsave(filename = "pdf-output/plots/arima_change_plot2.pdf",
...
ggsave(filename = "pdf-output/plots/arima_change_plot3.pdf",
...
![](plots/arima_change_plot1.pdf)

![](plots/arima_change_plot2.pdf)

![](plots/arima_change_plot3.pdf)
...
ggsave(filename = "pdf-output/plots/arima_all_residual_plots.pdf",
...
There was also an indication of non-stationarity for Violent Robbery. All other residuals were stationary.
...
ggsave(filename = "pdf-output/plots/arima_all_residual_plots_corrected.pdf",
...
ggsave(filename = "pdf-output/plots/lm_arima_percent_change_plot.pdf",
...
Conduct robustness checks for crime-mobility models, using pre/post lockdown data subsets.

```{r lm-arima-robustness-lockdown}
...
daily_crime_mobility_pre_lockdown %>%
  select(-c(`BRT + SCT passengers`)) %>%
  pivot_longer(-c(date, `BRT + SCT passengers (in millions)`),
               names_to = "Variable") %>%
  mutate(Variable = fct_inorder(Variable)) %>%
  ...
  refit(lm_arima_auto_mable,
        new_data = . ,
        reestimate = TRUE) -> lm_arima_auto_mable_pre_lockdown


daily_crime_mobility_post_lockdown %>%
  select(-c(`BRT + SCT passengers`)) %>%
  pivot_longer(-c(date, `BRT + SCT passengers (in millions)`),
               names_to = "Variable") %>%
  mutate(Variable = fct_inorder(Variable)) %>%
  ...
  refit(lm_arima_auto_mable,
        new_data = .,
        reestimate = TRUE) -> lm_arima_auto_mable_post_lockdown


lm_arima_auto_mable_pre_lockdown %>%
  tidy %>%
  mutate(Period = "Pre-lockdown") %>%
  bind_rows(lm_arima_auto_mable_post_lockdown %>%
              tidy %>%
              mutate(Period = "Post-lockdown")) -> lm_arima_auto_pre_post_lockdown_tidy

# Refit manual models

lm_manual_models_ls %>%
  map_df(~ .x %>% refit(new_data = daily_crime_mobility_pre_lockdown,
                     reestimate = TRUE) %>%
        tidy %>%
        mutate(Period = "Pre-lockdown"),
        .id = "Variable") -> lm_manual_pre_lockdown_tidy


lm_manual_models_ls %>%
  map_df(~ .x %>% refit(new_data = daily_crime_mobility_post_lockdown,
                     reestimate = TRUE) %>%
        tidy %>%
        mutate(Period = "Post-lockdown"),
        .id = "Variable") -> lm_manual_post_lockdown_tidy

lm_manual_pre_lockdown_tidy %>%
  bind_rows(lm_manual_post_lockdown_tidy) %>%
  mutate(Variable = factor(Variable, levels = var_names)) %>%
  bind_rows(lm_arima_auto_pre_post_lockdown_tidy) %>%
  arrange(Variable) -> lm_arima_all_models_pre_post_lockdown_tidy
...
```{r crime-mobility-coefficient-lockdown}

# get CI for mobility estimates


lm_arima_tidy_estimates %>%
  mutate(Period = "Pre- and post-lockdown") %>%
  bind_rows(lm_arima_all_models_pre_post_lockdown_tidy) %>%
  mutate(Period = fct_inorder(Period)) %>%
  filter(str_detect(term, "BRT")) %>%
  transmute(Variable = Variable,
            estimate = estimate,
            ...
            percent_upper = (mobility_drop^lower)-1,
            percent_lower = (mobility_drop^upper)-1,
            Period = Period) -> lm_arima_lockdown_ci_complete

lm_arima_lockdown_ci_complete %>%
  mutate(Variable = fct_rev(Variable)) %>%
  arrange(Variable) %>%
  ggplot(aes(percent_estimate, Variable, colour = Period)) +
  geom_point(position = position_dodge(width = -0.5))  +
  ...
  theme(legend.position = "bottom",
        ...
        legend.box = "vertical") +
  guides(colour = guide_legend(nrow=2,byrow=TRUE)) +
  labs(caption = "Error bars represent 95% confidence interval") -> lm_arima_percent_change_lockdown_plot

ggsave(filename = "pdf-output/plots/lm_arima_percent_change_lockdown_plot.pdf",
       plot = lm_arima_percent_change_lockdown_plot,
       device = cairo_pdf,
       width = 7,
       height = 6)
...
![](plots/lm_arima_percent_change_lockdown_plot.pdf)




Present robustness checks for crime-mobility models, using pre/post lockdown data subsets, excluding the period after the first cases were detected in the country.

```{r lm-arima-covid-robustness-lockdown}

# Combine pre covid and post lockdown auto estimates
...
lm_arima_auto_mable_pre_covid %>%
  ...
  bind_rows(lm_arima_auto_mable_post_lockdown %>%
              tidy %>%
              mutate(Period = "Post-lockdown")) -> lm_arima_auto_pre_covid_post_lockdown_tidy
...
lm_manual_pre_tidy %>%
  bind_rows(lm_manual_post_lockdown_tidy) %>%
  mutate(Variable = factor(Variable, levels = var_names)) %>%
  bind_rows(lm_arima_auto_pre_covid_post_lockdown_tidy) %>%
  arrange(Variable) -> lm_arima_all_models_pre_covid_post_lockdown_tidy
...
```{r crime-mobility-coefficient-covid-lockdown}

# get CI for mobility estimates


lm_arima_tidy_estimates %>%
  mutate(Period = "Complete period") %>%
  bind_rows(lm_arima_all_models_pre_covid_post_lockdown_tidy) %>%
  mutate(Period = fct_inorder(Period)) %>%
  filter(str_detect(term, "BRT")) %>%
  transmute(Variable = Variable,
            estimate = estimate,
            ...
            percent_upper = (mobility_drop^lower)-1,
            percent_lower = (mobility_drop^upper)-1,
            Period = Period) -> lm_arima_covid_lockdown_ci_complete

lm_arima_covid_lockdown_ci_complete %>%
  mutate(Variable = fct_rev(Variable)) %>%
  arrange(Variable) %>%
  ggplot(aes(percent_estimate, Variable, colour = Period)) +
  geom_point(position = position_dodge(width = -0.5))  +
  ...
  # scale_color_colorblind() +
  scale_color_manual(values = c("#000000", "#009E73", "#0072B2")) +
  scale_x_continuous("Estimated percentage change in crime for a 68.1% reduction in mobility",
                     labels = scales::percent) +
  ylab(NULL) +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 10),
        ...
        legend.box = "vertical") +
  guides(colour = guide_legend(nrow=2,byrow=TRUE)) +
  labs(caption = "Error bars represent 95% confidence interval") -> lm_arima_percent_change_covid_lockdown_plot

ggsave(filename = "pdf-output/plots/lm_arima_percent_change_covid_lockdown_plot.pdf",
       plot = lm_arima_percent_change_covid_lockdown_plot,
       device = cairo_pdf,
       width = 7,
       height = 6)
...
![](plots/lm_arima_percent_change_covid_lockdown_plot.pdf)
